...
class OTP:
    ...
    def __init__(self, start_point, end_point, mode, date, time, bike,
                 max_walk_distance=10000000000000000000000000000000000):
    ...
    def turn_into_trip(self, _id, user_id, trip_id):
        ...
        if "RAIL" in mode_list or "SUBWAY" in mode_list:  #
            ...
            try:
                ...
                cost = float(our_json['plan']['itineraries'][0]['fare']['fare']['regular'][
                                 'cents']) / 100.0  # gives fare in cents
            except:
                cost = 0
        elif "CAR" in mode_list:
            start_city_car = \
            str(Geocoder().reverse_geocode(car_start_coordinates.get_lat(), car_start_coordinates.get_lon())).split(
                ',')[1]
            end_city_car = \
            str(Geocoder().reverse_geocode(car_end_coordinates.get_lat(), car_end_coordinates.get_lon())).split(',')[1]
        ...
        return Alternative_Trip(_id, user_id, trip_id, sections, final_start_time, final_end_time, final_start_loc,
                                final_end_loc, 0, cost, mode_list)
